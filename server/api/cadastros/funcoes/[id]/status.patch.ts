...
export default defineEventHandler(async (event) => {
  ...
  const { data: atual } = await supabase.from('sgo_funcoes').select('status').eq('id', id).single()
  const novoStatus = atual?.status === 'ativo' ? 'inativo' : 'ativo'
  await supabase.from('sgo_funcoes').update({ status: novoStatus, updated_by: me.id }).eq('id', id)
